...
public:
   ...
   * returns <tt>FE_DGT<dim>(degree)</tt>, with @p dim and @p
  ...
  /**
   * Return the value @p ith shape function at the point @p p. See the
   ...
   * this function. This function is not implemented.
   */
  ...
  /**
   * Return the second derivative of the @p componentth vector component of the
   * @p ith shape function at the point @p p. See the FiniteElement base class
  ...
  /**
   * Return the polynomial degree of this finite element, i.e. the value passed
   * to the constructor.
   */
   ...
   *
   * For the FE_DGT class the result is always true (independent of the degree
   ...
   * meet at a common face, whether it is the other way around, whether neither
   * dominates, or if either could dominate.
   *
...
private:
  ...
  struct Matrices
  {
    ...
    /**
     * Number of elements (first index) the above field has. Equals the highest
     * polynomial degree plus one for which the embedding matrices have been
     * computed.
     */
...
private:
   ...
   * indicating what we want to know, return which of these need to be computed
   * each time we visit a new cell.
   *
